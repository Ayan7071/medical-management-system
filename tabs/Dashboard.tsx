...
import { TrendingUp, Package, Users, DollarSign, AlertCircle, X, ArrowRight } from 'lucide-react';
...
const Dashboard: React.FC<Props> = ({ medicines, transactions, patients }) => {
  ...
  const expiryAlerts = useMemo(() => {
    ...
    return (medicines || []).filter(med => {
      ...
      return isExpired || isSoon || isNextYearSoon;
    }).map(med => {
      const [m, y] = med.expiryDate.split('/').map(Number);
      const isExpired = y < currentYear || (y === currentYear && m <= currentMonth);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className="stat-card flex flex-col">
            ...
            {item.label === 'Out of Stock' && stats.outOfStock > 0 && (
              <button 
                onClick={() => setShowOutOfStock(true)}
                className="mt-4 w-full py-2 bg-rose-100 text-rose-600 rounded-lg text-xs font-bold hover:bg-rose-200 transition-all flex items-center justify-center gap-2"
              >
                View Out of Stock <ArrowRight size={14} />
              </button>
            )}
          </div>
        ))}
      ...
      {/* Out of Stock Modal/Section */}
      {showOutOfStock && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b flex items-center justify-between bg-rose-50">
              <div className="flex items-center gap-3">
                <AlertCircle className="text-rose-600" size={24} />
                <h3 className="text-xl font-bold text-slate-800">Out of Stock Medicines</h3>
              </div>
              <button onClick={() => setShowOutOfStock(false)} className="p-2 hover:bg-rose-100 rounded-full text-rose-400 transition-colors">
                <X size={24} />
              </button>
            </div>
                ...
                {(medicines || []).filter(m => m.stock === 0).map(med => (
                  <div key={med.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <div className="text-right">
                      <span className="px-3 py-1 bg-rose-100 text-rose-600 rounded-full text-[10px] font-black uppercase">Out of Stock</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="p-6 bg-slate-50 border-t flex justify-end">
              <button onClick={() => setShowOutOfStock(false)} className="px-8 py-3 bg-slate-800 text-white font-bold rounded-2xl">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            ...
            {medicines && medicines.length === 0 && <p className="text-center text-slate-400 py-10">No medicines added yet</p>}
            {medicines && medicines.length > 5 && <button className="w-full text-center text-sm text-blue-600 font-medium py-2">View All Inventory</button>}
          </div>
        </div>
      </div>
    </div>
